...
// Use like you want to, just don't blame me for anything.
  ...
  EntityTraitfeatherybutton = EntityBackbutton.extend({
	size: { x: 100, y: 50 },
	animSheet: new ig.AnimationSheet( 'media/traitFeathery.png', 100, 50 ),
	init: function(x, y, settings) {
		this.parent(x, y, settings);
		//any other init varialbles
	},
	update: function() {
		if ( Characterselectdatapass.Weightsto == false ) {
			this.setState( 'deactive' );
		}
		else if ( Characterselectdatapass.Weightsto ) {
			this.setState( 'idle' );
		}
		this.parent();
	},
	pressed: function() {
		Characterselectdatapass.Weightsto = false;
	},
	pressedUp: function() {
	}
